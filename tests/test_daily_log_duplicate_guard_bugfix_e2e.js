...
(async () => {
  ...
  await check("a first, genuinely new entry for a date saves immediately (no prior entry to collide with)", () => {
  ...
  await check("double-clicking '+ Add Daily Log' and submitting twice for the same date does NOT create a duplicate, but a second confirmed submit does add one", () => {
    win.PCC.router.go("dailylog");
  ...
  await check("changing the date after seeing the warning resets it — a genuinely different date doesn't need re-confirmation", () => {
    win.PCC.router.go("dailylog");
    win.PCC.router.render();
    findButtonByText(dom, "+ Add Daily Log").click();
    var dateInput = outlet().querySelector("#dlfield-log_date");
