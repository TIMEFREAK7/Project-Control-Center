...
function loadStoreWith(rawJsonString) {
  // setTimeout/clearTimeout: only the new getLastUsedName/rememberLastUsedName check
  // below actually calls store.update() (every prior check in this file only exercises
  // load()/migrate() or a pure newXxx() factory) — update()'s autosave debounce needs
  // window.setTimeout to exist, same as the real browser environment provides.
...
check("migrating a v46 dataset backfills schedule_performance_snapshots: [] — a brand new register, nothing to backfill on existing records", () => {
  const v46 = {
    schema_version: 46,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null },
  ...
  const store = loadStoreWith(JSON.stringify(v46));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.ok(Array.isArray(data.schedule_performance_snapshots));
  assert.strictEqual(data.schedule_performance_snapshots.length, 0);
});

check("newSchedulePerformanceSnapshot() produces a well-formed record with every performance field defaulting to null", () => {
  const store = loadStoreWith(null);
  const s = store.newSchedulePerformanceSnapshot({ project_id: "proj_1" });
  assert.ok(s.id);
  assert.strictEqual(s.project_id, "proj_1");
  assert.ok(s.captured_at);
  assert.strictEqual(s.spi, null);
  assert.strictEqual(s.cpi, null);
  assert.strictEqual(s.spi_t, null);
  assert.strictEqual(s.earned_schedule_days, null);
  assert.strictEqual(s.actual_time_days, null);
  assert.strictEqual(s.schedule_variance_days, null);
  assert.strictEqual(s.schedule_performance_score, null);
  assert.strictEqual(s.schedule_performance_rag, null);
  assert.strictEqual(s.schedule_progress_pct, null);
});

check("migrating a v47 dataset backfills lessons_learned: [] — a brand new register, nothing to backfill on existing records", () => {
  const v47 = {
    schema_version: 47,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null },
    ...
    documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [],
    schedules: [], wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
    cost_budget_items: [], cost_actuals: [],
    resources: [], resource_assignments: [], resource_unavailability: [],
    vendors: [], vendor_contacts: [], vendor_project_links: [], vendor_documents: [],
    vendor_meeting_links: [], vendor_rfi_links: [], vendor_risk_links: [], vendor_performance: [], vendor_notes: [],
  ...
  const store = loadStoreWith(JSON.stringify(v47));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.ok(Array.isArray(data.lessons_learned));
  assert.strictEqual(data.lessons_learned.length, 0);
});

check("newLessonLearned() produces a well-formed record defaulting category to 'other', impact_type to 'negative', and no status field", () => {
  const store = loadStoreWith(null);
  const l1 = store.newLessonLearned({ project_id: "proj_1", title: "Late rebar delivery" });
  const l2 = store.newLessonLearned({ project_id: "proj_1", title: "Precast sequencing worked well" });
  assert.ok(l1.id && l2.id && l1.id !== l2.id, "each lesson must get a unique id");
  assert.strictEqual(l1.project_id, "proj_1");
  assert.strictEqual(l1.category, "other");
  assert.strictEqual(l1.impact_type, "negative");
  assert.strictEqual(l1.description, "");
  assert.strictEqual(l1.recommendation, "");
  assert.strictEqual(l1.source_meeting_id, "");
  assert.strictEqual(l1.activity_id, "");
  assert.strictEqual(l1.status, undefined, "Lessons Learned is a log, not a workflow — no status field");
  assert.ok(l1.created_at);
});

check("LESSON_LEARNED_CATEGORIES and LESSON_LEARNED_IMPACT_TYPES are non-empty and include the expected values", () => {
  const store = loadStoreWith(null);
  ["schedule", "cost", "quality", "safety", "procurement_vendor", "design_technical", "communication", "other"].forEach((c) => {
    assert.ok(store.LESSON_LEARNED_CATEGORIES.indexOf(c) !== -1, "missing category: " + c);
  });
  ["positive", "negative"].forEach((i) => {
    assert.ok(store.LESSON_LEARNED_IMPACT_TYPES.indexOf(i) !== -1, "missing impact type: " + i);
  });
});

check("migrating a v48 dataset backfills knowledge_base_articles: [] — a brand new register, nothing to backfill on existing records", () => {
  const v48 = {
    schema_version: 48,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null },
    projects: [{ id: "proj_1", name: "Existing Project", archived: false, status: "on_track", progress: 0, attachments: [] }],
    documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    schedules: [], wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
    cost_budget_items: [], cost_actuals: [],
    resources: [], resource_assignments: [], resource_unavailability: [],
  ...
  const store = loadStoreWith(JSON.stringify(v48));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.ok(Array.isArray(data.knowledge_base_articles));
  assert.strictEqual(data.knowledge_base_articles.length, 0);
});

check("newKnowledgeBaseArticle() produces a well-formed record with project_id optional (empty by default) and no file attached", () => {
  const store = loadStoreWith(null);
  const a1 = store.newKnowledgeBaseArticle({ title: "Rebar Inspection Checklist" });
  const a2 = store.newKnowledgeBaseArticle({ title: "Precast Sequencing Guide" });
  assert.ok(a1.id && a2.id && a1.id !== a2.id, "each article must get a unique id");
  assert.strictEqual(a1.project_id, "", "portfolio-wide by default — no mandatory project");
  assert.strictEqual(a1.category, "other");
  assert.strictEqual(a1.body, "");
  assert.strictEqual(a1.tags, "");
  assert.strictEqual(a1.filename, "");
  assert.strictEqual(a1.file_size, 0);
  assert.strictEqual(a1.mime_type, "");
  assert.strictEqual(a1.status, undefined, "no status field — not a workflow");
  assert.ok(a1.created_at);
});

check("newKnowledgeBaseArticle() honors an explicit project_id when supplied", () => {
  const store = loadStoreWith(null);
  const a = store.newKnowledgeBaseArticle({ title: "Site-Specific Soil Report Guide", project_id: "proj_1" });
  assert.strictEqual(a.project_id, "proj_1");
});

check("KNOWLEDGE_BASE_CATEGORIES is non-empty and includes the expected values", () => {
  const store = loadStoreWith(null);
  ["standard_procedure", "checklist_template", "reference_material", "how_to_guide", "policy", "other"].forEach((c) => {
    assert.ok(store.KNOWLEDGE_BASE_CATEGORIES.indexOf(c) !== -1, "missing category: " + c);
  });
});

check("migrating a v49 dataset backfills document_reminder_due_soon_days=14 and action_centre_upcoming_days=30 onto settings, matching the values these used to be hardcoded to, and leaves already-set values untouched", () => {
  const v49 = {
    schema_version: 49,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
  ...
  const store = loadStoreWith(JSON.stringify(v49));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  ...
  // An install that had ALREADY set these (impossible pre-v50 in practice, but the
  // migration must still respect an explicit value over the default if one is present).
  const v49WithValue = JSON.parse(JSON.stringify(v49));
  v49WithValue.settings.document_reminder_due_soon_days = 21;
  v49WithValue.settings.action_centre_upcoming_days = 45;
  const store2 = loadStoreWith(JSON.stringify(v49WithValue));
  const data2 = store2.get();
  assert.strictEqual(data2.settings.document_reminder_due_soon_days, 21, "an already-set value must survive the migration untouched");
  assert.strictEqual(data2.settings.action_centre_upcoming_days, 45);
});

check("a brand-new install defaults document_reminder_due_soon_days to 14 and action_centre_upcoming_days to 30", () => {
  const store = loadStoreWith(null);
  const data = store.get();
  assert.strictEqual(data.settings.document_reminder_due_soon_days, 14);
  assert.strictEqual(data.settings.action_centre_upcoming_days, 30);
...
check("migrating a v50 dataset backfills report_templates: [] and empty company_logo_filename/mime_type — a brand new register and settings fields, nothing to backfill on existing records", () => {
  const v50 = {
    schema_version: 50,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30 },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [], wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
  ...
  const store = loadStoreWith(JSON.stringify(v50));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.ok(Array.isArray(data.report_templates));
  assert.strictEqual(data.report_templates.length, 0);
  assert.strictEqual(data.settings.company_logo_filename, "");
  assert.strictEqual(data.settings.company_logo_mime_type, "");
});

check("newReportTemplate() produces a well-formed record defaulting report_type to 'project' and sections to an empty object", () => {
  const store = loadStoreWith(null);
  const t1 = store.newReportTemplate({ name: "Client Report" });
  const t2 = store.newReportTemplate({ name: "Internal Report", report_type: "portfolio", sections: { kpis: true, risks: false } });
  assert.ok(t1.id && t2.id && t1.id !== t2.id, "each template must get a unique id");
  assert.strictEqual(t1.report_type, "project");
  assert.deepStrictEqual(t1.sections, {});
  assert.strictEqual(t2.report_type, "portfolio");
  assert.deepStrictEqual(t2.sections, { kpis: true, risks: false });
  assert.ok(t1.created_at);
});

check("REPORT_TEMPLATE_TYPES is exported and includes project/portfolio/management_pack", () => {
  const store = loadStoreWith(null);
  ["project", "portfolio", "management_pack"].forEach((t) => {
    assert.ok(store.REPORT_TEMPLATE_TYPES.indexOf(t) !== -1, "missing report template type: " + t);
  });
});

check("migrating a v51 dataset backfills settings.sidebar_collapsed: false — nothing else to backfill, no install has ever had this preference before", () => {
  const v51 = {
    schema_version: 51,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "" },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [], wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
  ...
  const store = loadStoreWith(JSON.stringify(v51));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.strictEqual(data.settings.sidebar_collapsed, false);

  // An install that had ALREADY set this (impossible pre-v52 in practice, but the
  // migration must still respect an explicit value over the default if one is present).
  const v51WithValue = JSON.parse(JSON.stringify(v51));
  v51WithValue.settings.sidebar_collapsed = true;
  const store2 = loadStoreWith(JSON.stringify(v51WithValue));
  const data2 = store2.get();
  assert.strictEqual(data2.settings.sidebar_collapsed, true, "an already-set value must survive the migration untouched");
});

check("a brand-new install defaults settings.sidebar_collapsed to false", () => {
  const store = loadStoreWith(null);
  const data = store.get();
  assert.strictEqual(data.settings.sidebar_collapsed, false);
});

check("migrating a v52 dataset backfills settings.density: 'comfortable' — matches every existing install's actual spacing exactly, so nothing visibly changes on upgrade", () => {
  const v52 = {
    schema_version: 52,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "" },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [], wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
  ...
  const store = loadStoreWith(JSON.stringify(v52));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.strictEqual(data.settings.density, "comfortable");

  // An install that had already set this somehow must keep its own value, not be
  // silently reset to the default — same "respect an explicit value" rule every prior
  // settings-backfill migration in this file follows.
  const v52WithValue = JSON.parse(JSON.stringify(v52));
  v52WithValue.settings.density = "compact";
  const store2 = loadStoreWith(JSON.stringify(v52WithValue));
  const data2 = store2.get();
  assert.strictEqual(data2.settings.density, "compact", "an already-set value must survive the migration untouched");
});

check("a brand-new install defaults settings.density to 'comfortable'", () => {
  const store = loadStoreWith(null);
  const data = store.get();
  assert.strictEqual(data.settings.density, "comfortable");
});

check("migrating a v53 dataset backfills settings.active_project_id: '' — matches how every existing install already behaves (no shared active project), so nothing visibly changes on upgrade", () => {
  const v53 = {
    schema_version: 53,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "" },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [], wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
  ...
  const store = loadStoreWith(JSON.stringify(v53));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.strictEqual(data.settings.active_project_id, "");

  // An install that had already set this somehow must keep its own value, not be
  // silently reset to the default — same "respect an explicit value" rule every prior
  // settings-backfill migration in this file follows.
  const v53WithValue = JSON.parse(JSON.stringify(v53));
  v53WithValue.settings.active_project_id = "some-project-id";
  const store2 = loadStoreWith(JSON.stringify(v53WithValue));
  const data2 = store2.get();
  assert.strictEqual(data2.settings.active_project_id, "some-project-id", "an already-set value must survive the migration untouched");
});

check("a brand-new install defaults settings.active_project_id to ''", () => {
  const store = loadStoreWith(null);
  const data = store.get();
  assert.strictEqual(data.settings.active_project_id, "");
});

check("migrating a v54 dataset backfills cpm_calculated_fingerprint: null onto existing schedules — Daily-Use Audit Phase 1's CPM-staleness fix, correctly reading as 'never calculated' rather than assumed fresh", () => {
  const v54 = {
    schema_version: 54,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "", active_project_id: "" },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [{ id: "sch_1", project_id: "proj_1", name: "Existing Schedule", revision_number: 0, version: "1.0", description: "", status: "draft", data_date: "2026-01-01" }],
    wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
    cost_budget_items: [], cost_actuals: [],
    resources: [], resource_assignments: [], resource_unavailability: [],
    vendors: [], vendor_contacts: [], vendor_project_links: [], vendor_documents: [],
  ...
  const store = loadStoreWith(JSON.stringify(v54));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.strictEqual(data.schedules.length, 1, "no schedules should be fabricated or dropped");
  assert.strictEqual(data.schedules[0].cpm_calculated_fingerprint, null, "an existing schedule with no recorded fingerprint must read as never-calculated, not assumed fresh");
});

check("a brand-new install defaults a new schedule's cpm_calculated_fingerprint to null", () => {
  const store = loadStoreWith(null);
  const schedule = store.newSchedule({ project_id: "proj_1", name: "Fresh Schedule" });
  assert.strictEqual(schedule.cpm_calculated_fingerprint, null);
});

check("migrating a v55 dataset backfills settings.last_used_names: {} — Daily-Use Audit Phase 3's field-memory feature, nothing to backfill since no install has ever remembered a name before this", () => {
  const v55 = {
    schema_version: 55,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "", active_project_id: "" },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [{ id: "sch_1", project_id: "proj_1", name: "Existing Schedule", revision_number: 0, version: "1.0", description: "", status: "draft", data_date: "2026-01-01", cpm_calculated_fingerprint: null }],
    wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [], schedule_performance_snapshots: [],
    cost_budget_items: [], cost_actuals: [],
    resources: [], resource_assignments: [], resource_unavailability: [],
  ...
  const store = loadStoreWith(JSON.stringify(v55));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.deepStrictEqual(data.settings.last_used_names, {});
});

check("a brand-new install defaults settings.last_used_names to {} and getLastUsedName()/rememberLastUsedName() round-trip", () => {
  const store = loadStoreWith(null);
  const data = store.get();
  assert.deepStrictEqual(data.settings.last_used_names, {});
  assert.strictEqual(store.getLastUsedName("rfi_raised_by"), "");
  store.rememberLastUsedName("rfi_raised_by", "  Priya Sharma  ");
  assert.strictEqual(store.getLastUsedName("rfi_raised_by"), "Priya Sharma", "value should be trimmed");
  store.rememberLastUsedName("rfi_raised_by", "   ");
  assert.strictEqual(store.getLastUsedName("rfi_raised_by"), "Priya Sharma", "a blank value must not overwrite a previously remembered one");
});

check("migrating a v56 dataset backfills settings.pinned_project_ids: [] — Daily-Use Audit Phase 5's pinned-projects feature, nothing to backfill since no install has ever pinned a project before this", () => {
  const v56 = {
    schema_version: 56,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "", active_project_id: "", last_used_names: {} },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [],
    wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
  ...
  const store = loadStoreWith(JSON.stringify(v56));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.deepStrictEqual(data.settings.pinned_project_ids, []);
});

check("a brand-new install defaults settings.pinned_project_ids to [] and projectContext's getPinnedIds()/isPinned()/togglePin() round-trip, filtering out archived/deleted projects", () => {
  const store = loadStoreWith(null);
  const data = store.get();
  assert.deepStrictEqual(data.settings.pinned_project_ids, []);

  const win = global.window;
  win.PCC.projectContext = win.PCC.projectContext || {};
  // projectContext.js isn't loaded by this file's own loadStoreWith() (only store.js
  // is eval'd there) — evaluate it now, against the same global.window, so its
  // functions share the just-loaded store.
  const fs2 = require("fs");
  const path2 = require("path");
  const src2 = fs2.readFileSync(path2.join(__dirname, "..", "src", "js", "projectContext.js"), "utf8");
  // eslint-disable-next-line no-eval
  eval(src2);

  const proj1 = store.newProject({ name: "Alpha" });
  const proj2 = store.newProject({ name: "Beta" });
  store.update((d) => {
    d.projects.push(proj1, proj2);
  });

  assert.strictEqual(win.PCC.projectContext.isPinned(proj1.id), false);
  win.PCC.projectContext.togglePin(proj1.id);
  assert.strictEqual(win.PCC.projectContext.isPinned(proj1.id), true);
  assert.deepStrictEqual(win.PCC.projectContext.getPinnedIds(), [proj1.id]);

  // Newly-pinned goes to the front.
  win.PCC.projectContext.togglePin(proj2.id);
  assert.deepStrictEqual(win.PCC.projectContext.getPinnedIds(), [proj2.id, proj1.id]);

  // Toggling an already-pinned project unpins it.
  win.PCC.projectContext.togglePin(proj2.id);
  assert.deepStrictEqual(win.PCC.projectContext.getPinnedIds(), [proj1.id]);

  // Archiving a pinned project drops it from getPinnedIds() without needing an
  // explicit unpin — same "re-validate on read, don't trust a stale id" rule get()
  // already follows for active_project_id.
  store.update((d) => {
    d.projects.find((p) => p.id === proj1.id).archived = true;
  });
  assert.deepStrictEqual(win.PCC.projectContext.getPinnedIds(), []);
});

check("migrating a v63 dataset backfills auto_generated:false and comments:[] onto every existing Delay Record — Auto Baseline Delay Detection + Bidirectional Delay Comments, both new fields a pre-existing record never had", () => {
  const v63 = {
    schema_version: 63,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "", active_project_id: "", last_used_names: {}, pinned_project_ids: [] },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [],
    wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
    delay_records: [
      { id: "dly_old1", activity_id: "act_1", project_id: "proj_1", status: "open", description: "Pre-existing delay, no auto_generated/comments fields at all" },
    ],
  ...
  const store = loadStoreWith(JSON.stringify(v63));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.strictEqual(data.delay_records.length, 1, "the pre-existing delay must survive the migration untouched otherwise");
  assert.strictEqual(data.delay_records[0].auto_generated, false);
  assert.deepStrictEqual(data.delay_records[0].comments, []);
  assert.strictEqual(data.delay_records[0].description, "Pre-existing delay, no auto_generated/comments fields at all");
});

check("migrating a v64 dataset backfills sync_mirror_enabled:false and sync_mirror_folder_path:\"\" onto settings — one-way hourly data mirror, both off by default so no existing install starts writing files anywhere without explicit opt-in", () => {
  const v64 = {
    schema_version: 64,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "", active_project_id: "", last_used_names: {}, pinned_project_ids: [] },
  ...
  const store = loadStoreWith(JSON.stringify(v64));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  assert.strictEqual(data.settings.sync_mirror_enabled, false);
  assert.strictEqual(data.settings.sync_mirror_folder_path, "");
  assert.strictEqual(data.settings.company_name, "", "an existing, unrelated setting must survive the migration untouched");
});

check("migrating a v65 dataset backfills ollama_enabled:false, ollama_host:\"http://localhost:11434\", ollama_model:\"\" onto settings — Ollama AI integration, off by default so no existing install starts calling out to a local server without explicit opt-in", () => {
  const v65 = {
    schema_version: 65,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", sidebar_collapsed: false, density: "comfortable", company_name: "Existing Co", backup_reminder_days: 7, backup_nudge_dismissed_at: null, document_reminder_due_soon_days: 14, action_centre_upcoming_days: 30, company_logo_filename: "", company_logo_mime_type: "", active_project_id: "", last_used_names: {}, pinned_project_ids: [], sync_mirror_enabled: false, sync_mirror_folder_path: "" },
    projects: [], documents: [], risks: [], daily_logs: [], meetings: [], rfis: [], change_orders: [], decisions: [], lessons_learned: [],
    knowledge_base_articles: [],
    schedules: [],
    wbs_items: [], activities: [], relationships: [], schedule_baselines: [], recovery_actions: [],
  ...
  const store = loadStoreWith(JSON.stringify(v65));
  const data = store.get();
  assert.strictEqual(data.schema_version, 66);
  ...
  assert.strictEqual(data.settings.company_name, "Existing Co", "an existing, unrelated setting must survive the migration untouched");
});
