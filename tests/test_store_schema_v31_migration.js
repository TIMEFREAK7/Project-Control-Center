...
// ---------------------------------------------------------------------------
check("a v20 dataset gets Gate 9 + Gate 10 + Gate 11 + Gate 13 + Gate 14 + Gate 15 + Gate 16 + Gate 17 + Gate 18 + Gate 5 + Gate 6 fields backfilled and lands on schema_version 31", () => {
  const v20 = {
    schema_version: 20,
  ...
  assert.strictEqual(data.schema_version, 31);
...
// ---------------------------------------------------------------------------
check("a v19 dataset gets activity_id backfilled onto existing budget items and lands on schema_version 31", () => {
  const v19 = {
    schema_version: 19,
    meta: { app_name: "x", created_at: "2026-01-01T00:00:00.000Z", last_saved_at: null, last_exported_at: null },
    settings: { theme: "dark", company_name: "", backup_reminder_days: 7, backup_nudge_dismissed_at: null },
...
// ---------------------------------------------------------------------------
// Full chain from a very old (v1-shaped) dataset still reaches v31 cleanly
// ---------------------------------------------------------------------------
check("a minimal legacy dataset (no schema_version at all) migrates all the way to 31 without throwing", () => {
  const legacy = {
