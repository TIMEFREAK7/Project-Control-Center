...
(function () {
  ...
  // Real bug found in the field (Aditya): a Status column driven by an Excel data-
  // validation dropdown reliably contains a human-readable label ("Not Started", "In
  // Progress", "Complete", "On Hold" — or a close variant, since template authors don't
  // all phrase the same list identically) rather than this app's own internal key
  // (not_started/in_progress/complete/on_hold, exactly what store.js's newActivity()
  // and ACTIVITY_STATUS_LABELS use). Before this alias table existed, parseRows() just
  // stored that raw label string verbatim as activity.status — which silently didn't
  // match any known status everywhere else in the app reads it (badges, filters, KPIs
  // all compare against the internal keys), so an imported activity's status looked
  // right in the spreadsheet but effectively vanished from every status-aware view in
  // PCC. Deliberately NOT the "conditional formatting" case (a cell fill color with no
  // effect on the cell's own text) — confirmed with Aditya via AskUserQuestion that's a
  // separate, genuinely unrecoverable limitation of the trimmed xlsx build this app
  // vendors (no cell-style parsing); this fixes the DATA the cell actually contains.
  // Lookup keys are pre-normalized by normalizeStatusToken() below (lowercased, and
  // -/_ collapsed to a single space) so "In-Progress"/"in_progress"/"In  Progress" all
  // resolve identically — same alias-table shape and "unrecognized → default + warn"
  // fallback ACTIVITY_TYPE_ALIASES above already established for Activity Type.
  var ACTIVITY_STATUS_ALIASES = {
  ...
  /** Same normalization ACTIVITY_STATUS_ALIASES' own keys are written in — lowercase,
   * trimmed, and every run of hyphens/underscores/whitespace collapsed to one space —
   * so a dropdown's "In-Progress", a hand-typed "in_progress", and "In   Progress" (extra
   * spacing from a copy-paste) all resolve to the exact same alias-table key. */
  ...
  function parseRows(headers, rows, columnMapping) {
    ...
    rows.forEach(function (row, rowIndex) {
      ...
      var statusRaw = String(rec.status || "").trim();
      var statusToken = normalizeStatusToken(statusRaw);
      var status = statusToken ? ACTIVITY_STATUS_ALIASES[statusToken] : "not_started";
      if (statusRaw && !status) {
        warnings.push({ row: lineNo, message: 'Unrecognized Status "' + statusRaw + '" \u2014 defaulted to Not Started.' });
        status = "not_started";
      }
